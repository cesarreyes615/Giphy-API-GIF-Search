var searchBarInput;
var searchBarText;
var giphyTrending = $.get('http://api.giphy.com/v1/gifs/trending?api_key=K6njTdQT89YOSYh0hr7AoZZAlaMYw61M&limit=5');
giphyTrending.done(function(response) { console.log("success got data", response);
...
document.getElementById("search").addEventListener('click',function(e){
    ...
    var giphySearch = $.get(`http://api.giphy.com/v1/gifs/search?api_key=K6njTdQT89YOSYh0hr7AoZZAlaMYw61M&q=${searchBarText}&limit=10&rating=g`);
    giphySearch.done(function(response){console.log('success got data',response);
...
document.getElementById('load-more').addEventListener('click', () => {
    var giphySearch = $.get(`http://api.giphy.com/v1/gifs/search?api_key=K6njTdQT89YOSYh0hr7AoZZAlaMYw61M&q=${searchBarText}&limit=15&offset=10&rating=g`);
    giphySearch.done(function(response){console.log('success got data',response);
        var giphySearchedData = response.data;
        var searchedGifsContainer = document.querySelector('.searched-gifs');
        var i;
